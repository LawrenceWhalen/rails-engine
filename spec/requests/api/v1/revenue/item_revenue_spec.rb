...
RSpec.describe 'items with most revenue' do
  before :each do
  ...
  describe 'get: /revenue/items' do
    it 'returns a specified number of items ordered by total revenue' do
      get '/api/v1/revenue/items?quantity=3'

      actual = JSON.parse(response.body, symbolize_names: true)

      expect(actual[:data].length).to eq(3)
      expect(actual[:data][0][:attributes][:name]).to eq(@item_3.name)
      expect(actual[:data][2][:attributes][:name]).to eq(@item_1.name)
    end
    it 'can return fewer than the number of eligable items' do
    ...
    it 'does not return items without real revenue' do
      get '/api/v1/revenue/items?quantity=20'
    ...
    it 'returns an empty array if no items have revenue' do
    ...
    it 'defaults to a quantity if non is provided' do
      ...
      expect(actual[:data].length).to eq(3)
      expect(actual[:data][0][:attributes][:revenue]).to eq(40.0)
      expect(actual[:data][1][:attributes][:revenue]).to eq(30.0)
      expect(actual[:data][2][:attributes][:revenue]).to eq(20.0)
    end
  end
